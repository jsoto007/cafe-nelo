"""Menu, daily specials and reservation-option content shared by the seed scripts
and the ``flask seed-if-empty`` CLI command.
...
Everything here runs inside an existing Flask app context.
"""
...
def seed_menu():
    """Replace the menu categories/items and daily specials with the seed content."""
    # Clear existing data
...
def seed_services():
    """Insert any reservation options that are missing (existing rows are left untouched)."""
    existing_names = {
