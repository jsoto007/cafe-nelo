...
export default function AdminUserDetails() {
  ...
  if (loadingUser) {
    return (
      <Card className="space-y-4">
        <h2 className="text-lg font-semibold text-gray-900 dark:text-gray-100">Loading user profile</h2>
        <p className="text-sm text-gray-600 dark:text-gray-300">Fetching the latest client information...</p>
      </Card>
    );
  }
  ...
  if (userError) {
    return (
      <Card className="space-y-4">
        <h2 className="text-lg font-semibold text-gray-900 dark:text-gray-100">Unable to display user</h2>
        <p className="text-sm text-gray-600 dark:text-gray-300">{userError}</p>
        <Button type="button" variant="secondary" onClick={handleBackToSettings}>
          Back to settings
        </Button>
      </Card>
    ...
    <div className="space-y-8">
      ...
      <Card className="space-y-6">
        <div>
          ...
          <div className="mt-3 grid gap-3 text-sm text-gray-700 dark:text-gray-200 sm:grid-cols-2">
            <div>
              <p className="text-xs uppercase tracking-[0.25em] text-gray-400 dark:text-gray-500">Email</p>
              <p className="font-medium text-gray-900 dark:text-gray-100">{userProfile.email || 'Not provided'}</p>
            </div>
            <div>
              <p className="text-xs uppercase tracking-[0.25em] text-gray-400 dark:text-gray-500">Phone</p>
              <p className="font-medium text-gray-900 dark:text-gray-100">{userProfile.phone || 'Not provided'}</p>
            </div>
            <div>
              <label
                htmlFor={roleOptions.length ? roleSelectId : undefined}
                className="text-xs uppercase tracking-[0.25em] text-gray-400 dark:text-gray-500"
              >
                Role
              </label>
              {roleOptions.length ? (
                <>
                  <select
                    ...
                    className="mt-2 w-full rounded-xl border border-gray-300 bg-white px-3 py-2 text-xs uppercase tracking-[0.2em] text-gray-700 shadow-sm focus:border-gray-900 focus:outline-none focus:ring-0 dark:border-gray-700 dark:bg-gray-950 dark:text-gray-200 dark:focus:border-gray-400"
                  >
                ...
                <p className="mt-2 font-medium uppercase tracking-[0.2em] text-gray-900 dark:text-gray-100">
                  {userProfile.role || 'user'}
                </p>
              )}
            </div>
            <div>
              <p className="text-xs uppercase tracking-[0.25em] text-gray-400 dark:text-gray-500">Member since</p>
              <p className="font-medium text-gray-900 dark:text-gray-100">{formatDate(userProfile.created_at)}</p>
            </div>
            <div>
              <p className="text-xs uppercase tracking-[0.25em] text-gray-400 dark:text-gray-500">Last seen</p>
        ...
        <div className="grid gap-3 sm:grid-cols-3">
          <div className="rounded-xl border border-gray-200 bg-white/80 p-4 text-sm shadow-inner dark:border-gray-800 dark:bg-gray-950/80">
            <p className="text-xs uppercase tracking-[0.25em] text-gray-400 dark:text-gray-500">Appointments</p>
            <p className="mt-2 text-2xl font-semibold text-gray-900 dark:text-gray-100">{userAppointments.length}</p>
            <p className="text-xs text-gray-500 dark:text-gray-400">Total recorded</p>
          </div>
          <div className="rounded-xl border border-gray-200 bg-white/80 p-4 text-sm shadow-inner dark:border-gray-800 dark:bg-gray-950/80">
            <p className="text-xs uppercase tracking-[0.25em] text-gray-400 dark:text-gray-500">Upcoming</p>
            <p className="mt-2 text-2xl font-semibold text-gray-900 dark:text-gray-100">{upcomingAppointments}</p>
            <p className="text-xs text-gray-500 dark:text-gray-400">Scheduled sessions</p>
          </div>
          <div className="rounded-xl border border-gray-200 bg-white/80 p-4 text-sm shadow-inner dark:border-gray-800 dark:bg-gray-950/80">
            <p className="text-xs uppercase tracking-[0.25em] text-gray-400 dark:text-gray-500">Assets</p>
            <p className="mt-2 text-2xl font-semibold text-gray-900 dark:text-gray-100">{totalFiles + totalNotes}</p>
            <p className="text-xs text-gray-500 dark:text-gray-400">Files & notes combined</p>
          </div>
        </div>
      </Card>
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        <Card className="space-y-4">
          <div>
            ...
            <p className="text-sm text-gray-600 dark:text-gray-300">Review booking history and open appointment records.</p>
          </div>
          {userAppointments.length ? (
            <ul className="space-y-3">
              {userAppointments.map((appointment) => (
                <li
                  key={appointment.id}
                  className="rounded-xl border border-gray-200 p-4 text-sm text-gray-700 shadow-sm dark:border-gray-800 dark:text-gray-200"
                >
              ...
              {userAssets.files.map(({ asset, appointment }) => (
                ...
                >
                  ...
                  <a
                    ...
                    className="mt-2 inline-flex text-xs font-semibold uppercase tracking-[0.25em] text-gray-900 transition hover:text-gray-600 dark:text-gray-100 dark:hover:text-gray-300"
                  >
                    Open file
                  </a>
                </li>
          ...
          <ul className="space-y-3">
            {userAssets.notes.map(({ asset, appointment }) => (
              ...
              >
                ...
                <p className="mt-2 whitespace-pre-wrap text-sm text-gray-700 dark:text-gray-200">{asset.note_text}</p>
